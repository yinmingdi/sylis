...
- compiler 的唯一正式输出；
- Lexicon Publisher 的唯一词典输入；
- 可供第三方离线消费的自描述数据集；
- 一次完整、不可变、可验证的 release build 输入。

它不是 API 的 `HeadwordDetail`，也不包含用户数据。API 按需要将关系数组投影为嵌套页面结构。

机器可读的 [JSON Schema 2020-12](./schemas/sylis-lexicon-artifact-v1.schema.json) 是字段、required/null、typed union 和 `additionalProperties` 的唯一结构真相。本页是与 Schema 对齐的人类可读说明，面向 compiler/publisher 开发者、API query module 和第三方离线消费者。`@sylis/lexicon-artifact` 从 Schema 生成 TypeScript 类型，并追加纯引用与业务语义验证器；compiler 与 publisher 都依赖该包，互不依赖。

除 `manifest.candidatePromotionLineage[].candidateRevisionId` 外，所有 `Id`/`*Id` 是由 artifact schema namespace、实体种类和稳定 identity key 确定性派生的 UUID；同一输入闭包跨机器、重跑和 release 复用同一 identity。`candidateRevisionId` 是已批准 revision 的不透明审计身份，只用于 Publisher 把 local projection 映射到正式实体；第三方可以保留或忽略它，不能据此读取候选 payload。显示文本、数组位置、随机 run ID 和数据库自增状态不得参与内容实体 ID，消费者也不得从 UUID 猜测实体类型，类型由所在 collection、lineage `entityType` 或 typed target discriminator 决定。
...
根对象及其七个成员都禁止未知字段。Schema 共有 111 个 array-valued property：`manifest.learningLanguageTags`、`manifest.inputs.sources` 与 `manifest.candidatePromotionLineage` 是三个元数据列表，其余 108 个是必须出现的实体集合；实体集合没有数据时输出 `[]`，不能省略。
...
`manifest.contentHash` 对解压后的 JSON 删除 `/manifest/contentHash` 后进行 canonicalization 并计算。Artifact 不包含运行时 BuildRun ID、数据库自增状态、审核内容或构建时间；唯一运维身份例外是已批准 `candidateRevisionId`，它与 local projection、正式 artifact UUID 一起构成 promotion lineage。相同固定输入、批准 revision、工具版本、模型响应缓存和配置产生相同 canonical bytes。物理 `.json.zst` 的 `artifactSha256`、byte size、构建时间和 BuildRun ID 属于 GitHub Release/BuildRun 运维元数据，不进入单文件内容。
...
```json
{
  ...
  "manifest": {
    ...
    "candidatePromotionLineage": [
      {
        "candidateRevisionId": "10000000-0000-4000-8000-000000000001",
        "localId": "example",
        "entityType": "EXAMPLE",
...
| `candidatePromotionLineage`    | 已批准 candidate revision 的 local projection 到 typed Artifact UUID 的确定性映射；不含 payload、审核人或模型请求 |
...
预算上限、token 单价、并发度、provider request ID、候选 payload/审核记录、物理文件 SHA-256、构建时间和 BuildRun ID 属于受保护 BuildRun/evidence，而不是词典内容。它们不进入 `manifest.contentHash`；已批准 revision 的不透明 ID 与 promotion lineage、prompt/schema/model policy、请求与实际模型身份会进入 manifest 和 candidate cache key。这样同一可复现输入仍生成相同内容，同时运维证据可以解释一次构建花费了多少以及怎样执行。
...
所有 collection 的准确名称、item 字段和 discriminator 由 JSON Schema 的 `$defs` 固定。Typed target 统一使用 `{ targetKind, targetId }`；Publisher 按 `targetKind` 拆到强外键表，不能从 ID 前缀猜类型。
...
| `sources.datasetVersions`        | `id`：版本 ID；`datasetId`：所属数据集；`version/sourceUri/checksum`：版本、下载位置和字节哈希；`retrievedAt`：获取时间；`adapter/parserVersion/schemaVersion`：解析器种类、实现版本和标准 candidate schema；`validationSummary`：该源的记录、错误、警告计数及验证器版本；`status`：公开 artifact 固定为 `VALIDATED`；`rightsPolicyId`：适用权利策略 |
...
| `sources.restrictions`           | `id`：限制事件 ID；`rightsPolicyId/datasetVersionId`：受影响策略和数据版本；`restrictionKind`：`BLOCK_BUILD/BLOCK_SERVE/BLOCK_EXPORT`；`reason/effectiveAt`：原因与生效时间                                                                                                                                                                          |
| `provenance.bundles`             | `id`：正式内容引用的 provenance ID；`kind`：`SOURCE/DERIVED/GENERATED/HUMAN`；`contentHash`：决策内容哈希；`resolverVersion`：合并/裁决算法版本；`decisionReason`：选用或合并这些证据的机器可读原因                                                                                                                                                  |
...
`ContentEvidence` 必须恰好指向一个原始 `SourceRecord` 或一个上游 `ContentProvenance`。bundle kind 与 evidence kind 必须匹配且图无环。AI 生成内容使用 `GENERATED`，但仍必须固定模型响应来源与 source-backed 上游证据，不能用空 provenance 表示“AI 猜的”；正式 Lexicon fact 不得引用 `GENERATED` bundle。

`adapter` 是带版本解析器注册表中的开放 code，`parserVersion/schemaVersion` 固定它在本次构建中的精确实现与输入 contract；它不属于数据库生命周期 enum。相反，`restrictionKind`、representation/media/concept/analysis/offset 等由 Sylis v1 自身定义的闭合集合必须通过 Schema enum 校验。
...
| `lexicon.conceptRevisions`        | `conceptId`：稳定概念；`conceptType`：`LOCAL_SENSE/SYNSET`；`provenanceId`：来源                                                                                                                            |
...
| `lexicon.morphology.analyses`                  | `id/formRepresentationId`：被分析的具体词形表示；`analysisType`：`INFLECTION/DERIVATIONAL`；`provenanceId`：来源                                                                                          |
...
| `lexicon.corpora.attestations`                 | `id/datasetVersionId/target`：语料实例；`documentRef/offset/surfaceText`：文档位置和表层文本；`offsetUnit`：`UTF8_BYTE/UNICODE_CODE_POINT/UTF16_CODE_UNIT`；`sourceRecordId/provenanceId`：原始记录和来源 |
...
| `gradingMode`            | `EXACT/WEIGHTED/SELF_REPORT`；SELF_REPORT 只允许练习；0.0.1 不发布 AI 自动评分                                     |
| `validationLevel`        | `PRACTICE_ONLY/FORMATIVE_VERIFIED/SUMMATIVE_VERIFIED`；EXTENDED_TEXT、NO_CAPTURE、SELF_REPORT 强制 `PRACTICE_ONLY` |
...
2. 在 compressed/decompressed byte 上限与 compression-ratio 上限内流式解压，只接受一个标准 zstd frame，拒绝 skippable/multiple frame、trailing data、截断和资源耗尽输入。compiler v1 默认上限为 512 MiB compressed、512 MiB decompressed、200:1；Builder/Publisher 可以收紧但不能放宽已批准 manifest 的限制。
...
compiler 和 publisher 使用 `@sylis/lexicon-artifact` 导出的同一 schema/type，不各自复制接口。CI 还会把 schema 中的所有数组与 publisher mapping registry 双向比较，避免新增数据被静默忽略。第三方只需 JSON Schema 即可消费，不要求安装 NestJS、Prisma 或 compiler。

## 13. 单文件与大数据处理

Kaikki 使用 JSONL 是因为完整 Wiktionary 数据无法整体载入内存；Sylis 的目标是约 2 万核心学习 lemma 的精选 release，但仍不得 `JSON.stringify()` 整个对象。writer 按顶层数组流式产生 canonical JSON bytes，并直接接入单一 zstd stream；validator/publisher 以相反方向流式解压到 JSON parser 和 disk-backed ID index，不落地第二份完整 `.json`。一个文件是交换要求，不代表一次性加载。
