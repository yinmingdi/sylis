...
import type { GetCurrentBookResDto } from '@/legacy-dto';
...
const WordPage = () => {
  ...
  const learnedWords = Math.floor(
    ((data?.progress ?? 0) * (data?.totalWords ?? 0)) / 100,
  );
  ...
  const renderBookProgress = () => (
    <>
      <div className={styles.bookMeta}>
        <span>
          {learnedWords}/{data?.totalWords ?? 0}词
        </span>
  ...
  const renderEmptyBookState = () => (
    <div className={styles.emptyBookState}>
      ...
      <p>暂未选择词书</p>
      <Button color="primary" size="small" onClick={() => navigate('/books')}>
        选择词书
      </Button>
    </div>
  );
  ...
  const renderBookCard = () => (
    <div className={styles.bookSection}>
      <Card className={styles.bookCard}>
        {loading ? (
          <Skeleton.Paragraph
            className={styles.bookCardSkeleton}
            lineCount={3}
            animated
          />
        ) : data?.book ? (
