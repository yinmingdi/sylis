...
import { CollectionSource } from '@/legacy-dto';
...
const WordDetailPage = () => {
  ...
  useEffect(() => {
    ...
    vocabularyNotebookApi
      .isWordInDefaultNotebook(wordData.id)
      .then((isCollected) => {
        wordCollection.setCollected(isCollected ? [wordData.id] : []);
      })
      .catch((error) => {
        console.error('获取收藏状态失败:', error);
      });
  }, [wordData?.id, wordCollection.setCollected]);
  ...
  const actions = [
    {
      label: wordCollection.isCollected(wordData?.id || '')
        ? '已收藏'
        : '加入生词本',
  ...
  return (
    <PageView
      ...
      appBar={<AppBar title="单词详情" onBack={handleBack} />}
    >
