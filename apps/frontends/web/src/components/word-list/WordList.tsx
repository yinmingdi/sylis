...
const WordList: React.FC<WordListProps> = ({
        ...
        <div className={styles.wordHeader}>
            ...
            <div className={styles.wordActions}>
              ...
              {showCollectButton && (
                <div
                  ...
                  onClick={(e) =>
                    handleToggleCollect(e, word.id, word.isCollected || false)
                  }
                >
                  {word.isCollected ? (
          ...
          <div className={styles.wordMeta}>
            <div className={styles.badges}>
              ...
              {showProficiency && word.proficiencyScore !== undefined && (
                <Badge
                  content={`熟练度 ${word.proficiencyScore}`}
                  style={
                    {
              ...
              {showProficiency &&
                word.accuracyRate !== undefined &&
                word.reviewCount &&
                word.reviewCount > 0 && (
                  <Badge
                    content={`正确率 ${(word.accuracyRate * 100).toFixed(0)}%`}
                    style={
                      {
                        backgroundColor:
                          word.accuracyRate >= 0.8
                            ? '#06d6a0'
                            : word.accuracyRate >= 0.6
                              ? '#ff9f1c'
                              : '#f71735',
                        color: '#fff',
                      } as React.CSSProperties
  ...
  return (
    <div
      className={`${styles.wordList} ${layout === 'list' ? styles.listLayout : ''}`}
    >
      {layout === 'card' ? (
        <Grid columns={1} gap={12}>
          {words.map((word) => (
            <Grid.Item key={word.id}>{renderWordCard(word)}</Grid.Item>
          ))}
        </Grid>
      ) : (
        <div className={styles.listContainer}>{words.map(renderWordList)}</div>
      )}
    </div>
  );
};
