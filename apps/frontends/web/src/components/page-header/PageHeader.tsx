...
const PageHeader: React.FC<PageHeaderProps> = ({
  ...
  return (
    <div className={`${styles.pageHeader} ${className}`}>
      {hasLeftContent && (
        <div className={styles.left}>{renderLeftActions()}</div>
      )}

      <div
        className={`${styles.center} ${shouldCenterTakeFullWidth ? styles.centerFullWidth : ''}`}
      >
        {children ? children : <div className={styles.title}>{title}</div>}
      </div>

      {hasRightContent && <div className={styles.right}>{renderActions()}</div>}
    </div>
  );
};
