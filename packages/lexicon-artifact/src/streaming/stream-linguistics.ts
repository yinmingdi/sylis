...
export class StreamingLinguisticValidator {
  ...
  accept(collectionPath: ArtifactCollectionPath, value: unknown): void {
    switch (collectionPath) {
      ...
      case ArtifactCollectionPath.SENSE_CONCEPT_MEMBERSHIPS: {
      ...
      case ArtifactCollectionPath.MORPHOLOGICAL_SEGMENTS: {
  ...
  issues(): LinguisticIssue[] {
    ...
    for (const senseId of this.#senses.keys()) {
      const count = this.#canonicalConceptCounts.get(senseId) ?? 0;
      if (count === 0) {
        issues.push({
          code: "MISSING_CANONICAL_CONCEPT",
          message: `Sense ${senseId} has no canonical Concept.`,
          entityId: senseId,
        });
      } else if (count > 1) {
        issues.push({
