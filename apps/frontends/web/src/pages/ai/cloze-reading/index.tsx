import { Button, Card, Toast, Divider } from 'antd-mobile';
import React, {
  useState,
  useCallback,
  useEffect,
...
import { AiOutlineReload, AiOutlineBook } from 'react-icons/ai';
...
import {
  splitIntoSentences,
  tokenizeText,
} from '../../../components/interactive-text/utils/textParser';
...
const ClozeReadingPage: React.FC = () => {
  ...
  const [answerWidths, setAnswerWidths] = useState<Map<string, number>>(
    new Map(),
  );
  // 存储每个单词应该隐藏的位置标识符 (word -> positionKey)
  const [hiddenPositions, setHiddenPositions] = useState<Map<string, string>>(
    new Map(),
  );

  // 解析文章并找出每个单词的所有出现位置，然后随机选择一个位置
  const findAndSelectRandomPositions = useCallback(
    (content: string, wordsToTest: Word[]) => {
      // 解析文章内容
    ...
    try {
      ...
      if (result.data) {
        ...
        const article: Article = {
          ...
          words:
            articleData.usedWords?.map((word: string, index: number) => ({
              id: `word_${index}`,
              word: word,
        ...
        const wordsToTest = article.words.slice(
          0,
          Math.min(5, article.words.length),
        ); // 最多5个单词
  ...
  // 获取单词的填空状态
  const getBlankByWord = useCallback(
    (word: string): ClozeBlank | undefined => {
  ...
  const handleSubmitTest = useCallback(() => {
    const unansweredCount = clozeBlanks.filter(
      (blank) => !blank.userAnswer,
    ).length;
  ...
  const renderClozeToken = useCallback(
    (
      ...
      if (token.type === 'word') {
        const lowerWord = token.content.toLowerCase();
        const positionKey =
          paragraphIndex !== undefined && sentenceIndex !== undefined
        ...
        const shouldHide =
          positionKey &&
          hiddenPositions.has(lowerWord) &&
          hiddenPositions.get(lowerWord) === positionKey;
            ...
            <span key={index} className={styles.spellingBlank}>
              ...
              <SpellingInput
                ...
                onEditAfterSubmit={() => {
                  setIsSubmitted(false);
                  setClozeBlanks((prev) =>
                    prev.map((b) => ({ ...b, isCorrect: undefined })),
                  );
                }}
              />
