...
import type { QuizChoiceDataDto, QuizWordInfoDto } from '@/legacy-dto';
...
const meta: Meta<typeof WordQuizChoice> = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component:
          '单词选择题组件，用户需要根据单词选择正确的释义。支持答案反馈、美音/英音切换和语音播放。',
      },
    },
  },
  argTypes: {
...
export const WithCallbacks: Story = {
  args: {
    ...
    onAnswer: (selectedWordId, isCorrect) => {
      console.log(
        `选择的单词ID: ${selectedWordId}, 答案${isCorrect ? '正确' : '错误'}`,
      );
    },
    onPlayPronunciation: () => {
      console.log('播放单词发音');
  ...
  render: () => {
    ...
    const handleAnswer = (_selectedWordId: string, isCorrect: boolean) => {
      ...
      if (currentIndex < quizzes.length - 1) {
        ...
        setTimeout(() => {
          console.log(
            `练习完成！得分: ${isCorrect ? score + 1 : score}/${quizzes.length}`,
          );
        }, 1500);
      }
    };
    ...
    return (
      <div>
        ...
        <WordQuizChoice
          ...
          onPlayPronunciation={() =>
            console.log(`播放 ${currentQuiz.word.headword} 的发音`)
          }
        />
      </div>
    );
...
export const WithViewAnswer: Story = {
  render: () => {
    ...
    const handleAnswer = (
      ...
      console.log(
        `选择的单词ID: ${selectedWordId}, 答案${isCorrect ? '正确' : '错误'}, ${isCheated ? '已查看答案' : '正常作答'}`,
      );
      setAnswered(true);
    };
      ...
      <div>
        ...
        >
          ...
          {answered && quizRef.current?.isCheated && (
            <span style={{ color: '#ff006e', fontSize: '14px' }}>
              ⚠️ 已查看答案，本题不计分
            </span>
          )}
        </div>
