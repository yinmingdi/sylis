...
import type { CollectedWordItemDto } from '@sylis/shared/dto';
import {
  Button,
  SearchBar,
  PullToRefresh,
...
const VocabularyBookPage: React.FC = () => {
  ...
  const loadVocabularyData = useCallback(
    async (isRefresh = false, page = 1) => {
      try {
        ...
        const res = await vocabularyNotebookApi.getNotebookWords(notebookId, params);
        const convertedWords = res.data.words.map(convertToWordItem);

        // 维护本地全集并基于本地数据进行搜索过滤
        setAllWords((prev) => {
          const nextAll = page === 1 ? convertedWords : [...prev, ...convertedWords];
        ...
        // 获取统计信息
        const statsRes = await vocabularyNotebookApi.getNotebookStats(notebookId);
        setStats({
  ...
  const handleToggleCollect = async (wordId: string, collected: boolean) => {
    try {
      if (!collected && defaultNotebookId) {
        // 从生词本移除
        await vocabularyNotebookApi.removeWordFromNotebook(defaultNotebookId, wordId);
    ...
    try {
      ...
      await Promise.all(
        selectedWords.map((wordId) =>
          vocabularyNotebookApi.removeWordFromNotebook(defaultNotebookId, wordId),
        ),
      );

      setWords((prev) => prev.filter((word) => !selectedWords.includes(word.id)));
      setAllWords((prev) => prev.filter((word) => !selectedWords.includes(word.id)));
      setStats((prev) => ({
        ...prev,
  ...
  // 处理文章生成完成
  const handleArticleComplete = useCallback((articleId: string) => {
    setEditMode(false);
    setSelectedWords([]);
    // 可以跳转到文章详情页
  ...
  if (loading && !refreshing) {
    return (
      <PageView
        appBar={
          <AppBar title="生词本" onBack={() => navigate(-1)} />
        }
  ...
  return (
    ...
    >
      <PullToRefresh onRefresh={handleRefresh}>
        ...
        {words.length === 0 ? (
          <ErrorBlock description="生词本为空" image="/images/empty-vocabulary.png">
            <Button color="primary" onClick={() => navigate('/vocabulary-learning')}>
              去学习单词
            </Button>
          </ErrorBlock>
        ) : (
