...
- `tools/engineering-harness`：通用生成器、checker、测试、assets 和可选 Skill。

`tools/engineering-harness` 中的 Skill 不会自动安装到 `.agents/skills`，也不会启用自定义代理或模型路由。当前项目只使用确定性 CLI 和生成的仓库文件。
...
```bash
pnpm --filter @sylis/engineering-harness run init -- \
...
Sylis 当前允许 root、`apps/frontends/*`、`apps/backends/*` 和 docs 使用 `packages/*`；packages 可以在明确 allowlist 内使用其他 packages；consumer workspace 之间不能直接依赖。所有内部版本必须使用 `workspace:`，并禁止重复 package 名和依赖环。
