...
import type { GetBooksResDto } from '@/legacy-dto';
...
const BooksPage: React.FC = () => {
  ...
  const stats = useMemo((): BookStats => {
    const totalWords = books.reduce(
      (sum, book) => sum + (book.wordNum || 0),
      0,
    );
    const popularBooks = books.filter(
      (book) => (book.reciteUserNum || 0) > 100,
    ).length;
  ...
  const filteredBooks = useMemo(() => {
    return books.filter((book) => {
      ...
      const matchesSearch =
        ...
        (book.introduce &&
          book.introduce.toLowerCase().includes(search.toLowerCase()));
