...
export class LearningService {
  ...
  async getCurrentBook(userId: string): Promise<GetCurrentBookResDto> {
    ...
    if (userLearning && userLearning.currentBookId) {
      ...
      // 查询用户该词书的学习进度
      const userBook = await this.getUserBook(userId, userLearning.currentBookId);
      if (userBook && currentBook) {
        newWords = userBook.dailyNewWords || 0;
        totalWords = currentBook.wordNum || 0;
        // 统计已学单词数
        const learnedWordsNum = await this.learningRepository.getLearnedWordsCount(
          userLearning.id,
          userLearning.currentBookId,
        );
        progress = totalWords ? Math.round((learnedWordsNum / totalWords) * 100) : 0;
      }
    }
    return {
      daysLeft,
