import { Button, Input, Avatar, Toast, Dialog, Form } from 'antd-mobile';
...
const Profile = () => {
  ...
  const handleAvatarChange = async (files: File[]) => {
    if (files.length > 0) {
      ...
      try {
        ...
        Toast.show(errorMessage(error, '头像上传失败，请重试'));
  ...
  // 处理字段编辑
  const handleFieldEdit = (field: ProfileField) => {
    setEditMode((prev) => ({
      ...prev,
      [field]: true,
    }));
  ...
  // 保存字段修改
  const handleFieldSave = async (field: ProfileField, value: string) => {
    if (!value.trim()) {
      Toast.show(
        `${field === ProfileField.NICKNAME ? '昵称' : '邮箱'}不能为空`,
      );
      return;
    }
    ...
    try {
      ...
      Toast.show(
        `${field === ProfileField.NICKNAME ? '昵称' : '邮箱'}更新成功`,
      );
    } catch (error) {
      Toast.show(errorMessage(error, '更新失败，请重试'));
  ...
  // 取消编辑
  const handleFieldCancel = (field: ProfileField) => {
    setEditMode((prev) => ({
      ...prev,
      [field]: false,
    }));
  ...
  const handlePasswordSubmit = async () => {
    try {
      ...
      Toast.show(errorMessage(error, '密码修改失败，请重试'));
    ...
    const personalFeatures = [
      {
        ...
        icon: <AiOutlineUser />,
        color: '#2ec4b6',
        onClick: () => handleFieldEdit(ProfileField.NICKNAME),
      },
      {
        id: 'edit-email',
        title: '修改邮箱',
        ...
        icon: <AiOutlineMail />,
        color: '#3b82f6',
        onClick: () => handleFieldEdit(ProfileField.EMAIL),
      },
      {
        id: 'change-password',
        title: '修改密码',
              ...
              <Button
                ...
                onClick={(e) => {
                  const input = e.currentTarget
                    .closest(`.${styles.editCard}`)
                    ?.querySelector<HTMLInputElement>('input');
                  if (input) handleFieldSave(field, input.value);
                }}
              >
                <AiOutlineCheck />
                保存
  ...
  if (initialLoading) {
    return (
      <PageView
        ...
        appBar={<AppBar title="个人中心" onBack={() => navigate(-1)} />}
      >
    ...
    >
      <div className={styles.content}>
        ...
        <div className={styles.logoutSection}>
          <div className={styles.logoutCard} onClick={handleLogout}>
