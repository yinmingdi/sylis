...
export class BooksService {
  ...
  async edition(bookId: string, editionId: string, query: BookEditionQueryDto) {
    ...
    const targets = items.map(bookItemAnchor);
    const idsByKind = new Map<string, string[]>();
    for (const target of targets) {
      const ids = idsByKind.get(target.targetKind) ?? [];
      ids.push(target.targetId);
      idsByKind.set(target.targetKind, ids);
    }
    const [headwords, entries] = await Promise.all([
    ...
    return {
      ...
      data: {
        ...
        items: items.map((item, index) => {
          ...
          if (!target) throw new Error(`BOOK_ITEM_TARGET_MISSING:${item.id}`);
          return {
            id: item.id,
            ...
            ...target,
            ...(labels.get(
              learningTargetKey(target.targetKind, target.targetId),
            ) ?? { displayText: target.targetId }),
          };
        }),
