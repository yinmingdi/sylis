import type { SearchWordResDto } from '@sylis/shared/dto';
import React, {
  Fragment,
  type KeyboardEvent,
  useCallback,
...
const WordSelector: React.FC<WordSelectorProps> = ({
  ...
  const fetchSuggestions = useCallback(async (keyword: string) => {
    ...
    try {
      ...
      // 转换 API 响应格式为组件需要的格式
      const convertedSuggestions: WordSuggestion[] = results.map((word: SearchWordResDto) => ({
        id: word.id,
        word: word.headword,
  ...
  const onCompositionEnd = useCallback((event: any) => {
        ...
        onChange={() => {
          ...
          if (selection && Range.isCollapsed(selection)) {
            ...
            if (foundTrigger && triggerOffset) {
              ...
              const match = searchText.match(new RegExp(`^${triggerChar}([^\\n\\r]*)$`));
...
const Word = ({
  ...
  const style: React.CSSProperties = {
    ...
    boxShadow: selected && focused ? '0 0 0 2px var(--color-primary-300)' : 'none',
