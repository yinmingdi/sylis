...
import { lexiconBuilderConfigFromEnv } from "./config/builder-config";
import { createLexiconBuildHandler } from "./handlers/lexicon-build-handler";
...
async function main(): Promise<void> {
  ...
  const health = startWorkerHealthServer({
    port: config.port,
    service: JobWorkerService.LEXICON_BUILDER,
    state: () => state,
  ...
  try {
    await runJobWorker({
      ...
      kinds: [JobKind.LEXICON_BUILD],
      handle: createLexiconBuildHandler(database, config),
      signal: abort.signal,
