...
} from "../validate/profiles";
import { DiskBackedReferenceIndex } from "../validate/reference-index";
import { publicArtifactRightsViolation } from "../validate/source-rights";
import { StreamingLinguisticValidator } from "../validate/stream-linguistics";
import { assertValidationSummary } from "../validate/validation-summary";
...
const PROFILE_COLLECTIONS = {
  ...
  "/quality/profileVersions": "profileVersions",
  "/quality/profileEvaluations": "profileEvaluations",
  "/quality/profileEvaluationTargets": "profileEvaluationTargets",
  "/quality/coverage": "coverage",
} as const satisfies Record<string, keyof ContentProfileReport>;
...
export async function validateArtifactStream(
  ...
  try {
    ...
    const actualProfileDigests = new Map(
      Object.keys(PROFILE_COLLECTIONS).map((path) => [
        path,
        createHash("sha256"),
      ]),
    );
    ...
    parser.onValue = ({ value, key, stack }) => {
      ...
      if (collection.manifestPath === "/sources/rightsPolicies") {
      ...
      if (typeof value === "object" && value !== null) {
        ...
        if (collection.manifestPath === "/lexicon/entryRevisions") {
        ...
        } else if (collection.manifestPath === "/lexicon/senseRevisions") {
        ...
        } else if (collection.manifestPath === "/lexicon/headwordRevisions") {
          const candidate = {
        ...
        } else if (collection.manifestPath === "/quality/sourceStatistics") {
          sourceStatistics.push(value as unknown as NamedCount);
        } else if (collection.manifestPath === "/quality/exerciseStatistics") {
          exerciseStatistics.push(value as unknown as NamedCount);
        } else if (collection.manifestPath === "/sources/datasets") {
          sourceDatasetKeys.set(String(item.id), String(item.key));
        } else if (collection.manifestPath === "/sources/datasetVersions") {
    ...
    for (const [path, reportKey] of Object.entries(PROFILE_COLLECTIONS)) {
