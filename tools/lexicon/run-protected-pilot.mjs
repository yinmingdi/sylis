...
import { assertCleanCommit } from "./assert-clean-commit.mjs";

const workspaceRoot = resolve(dirname(fileURLToPath(import.meta.url)), "../..");
const outputRoot = resolve(workspaceRoot, ".work/lexicon-pilot");
...
assertCleanCommit(workspaceRoot);
...
function assertPilotResults(noAiValidation, firstValidation, secondValidation) {
  ...
  const sourceMaterialKinds = [
    "LEARNER_EXPLANATION",
    "MORPHOLOGY_WALKTHROUGH",
    "CULTURAL_CONTEXT",
  ];
  const aiMaterialKinds = [...sourceMaterialKinds, "MNEMONIC", "MICRO_STORY"];
  for (const [name, result, requiredKinds] of [
    ["no-AI", noAiValidation, sourceMaterialKinds],
    ["first AI", firstValidation, aiMaterialKinds],
    ["cached AI", secondValidation, aiMaterialKinds],
  ]) {
  ...
  return { firstBytes, firstAiArtifactManifest };
}

function reviewRequestFor(firstValidation, pilotStateSha256) {
  return {
    reviewVersion: "sylis.lexicon-pilot-review/1",
    commit,
    pilotStateSha256,
...
if (!reviewFile) {
  ...
  const state = {
    stateVersion: "sylis.lexicon-pilot-state/1",
    commit,
    sourceManifest,
    buildConfiguration,
    noAi: {
  ...
  throw new Error(
    `Manual sample review is required. Review ${reviewRequestPath}, create an approval file outside ${outputRoot}, set LEXICON_PILOT_REVIEW_FILE, and rerun pnpm lexicon:pilot. The approval pass revalidates the frozen artifacts without calling the provider.`,
  );
}
...
if (!existsSync(statePath)) {
  throw new Error(
    "Protected pilot state is missing. Run pnpm lexicon:pilot once without LEXICON_PILOT_REVIEW_FILE before submitting approval.",
  );
}
const state = readJson(statePath);
const frozenStateSha256 = artifactSha256(statePath);
if (
  state.stateVersion !== "sylis.lexicon-pilot-state/1" ||
  state.commit !== commit ||
...
const reviewedSampleIds = Array.isArray(review.reviewedHeadwordIds)
  ...
  : [];
if (
  review.reviewVersion !== "sylis.lexicon-pilot-review/1" ||
...
const evidence = {
  evidenceVersion: "sylis.lexicon-pilot-evidence/3",
  commit,
  sourceManifest,
  buildConfiguration: {
...
process.stdout.write(
  `\nProtected exact-200 lexicon pilot passed. Evidence: ${evidencePath}\n`,
);
