...
const ArticleGenerator: React.FC<ArticleGeneratorProps> = ({
      ...
      <div className={styles.section}>
        ...
        <div className={styles.wordSourceToggle}>
          <button
            ...
            onClick={() => !disabled && setConfig(prev => ({ ...prev, useWeakWords: false }))}
            disabled={disabled}
          >
          ...
          <button
            type="button"
            className={`${styles.toggleOption} ${config.useWeakWords ? styles.active : ''}`}
            onClick={() => !disabled && setConfig(prev => ({ ...prev, useWeakWords: true }))}
            disabled={disabled}
          >
    ...
    return (
      <div className={styles.section}>
        ...
        <Picker
          columns={[THEMES]}
          value={[config.theme]}
          onConfirm={(value) => !disabled && setConfig(prev => ({ ...prev, theme: value[0] as any }))}
        >
          {(_, actions) => (
            <div className={styles.pickerTrigger} onClick={disabled ? undefined : actions.open}>
  ...
  const renderDifficultySelector = () => {
    const selectedDifficulty = DIFFICULTIES.find(difficulty => difficulty.value === config.difficulty);
    ...
    return (
      <div className={styles.section}>
        ...
        <Picker
          ...
          onConfirm={(value) => !disabled && setConfig(prev => ({ ...prev, difficulty: value[0] as any }))}
        >
          {(_, actions) => (
            <div className={styles.pickerTrigger} onClick={disabled ? undefined : actions.open}>
              <span className={styles.pickerLabel}>{selectedDifficulty?.label}</span>
              <span className={styles.pickerDescription}>{selectedDifficulty?.description}</span>
              <span className={styles.pickerArrow}>▼</span>
            </div>
          )}
        </Picker>
  ...
  const renderLengthSelector = () => {
    const selectedLength = LENGTHS.find(length => length.value === config.length);
    ...
    return (
      <div className={styles.section}>
        ...
        <Picker
          columns={[LENGTHS]}
          value={[config.length]}
          onConfirm={(value) => !disabled && setConfig(prev => ({ ...prev, length: value[0] as any }))}
        >
          {(_, actions) => (
            <div className={styles.pickerTrigger} onClick={disabled ? undefined : actions.open}>
              <span className={styles.pickerLabel}>{selectedLength?.label}</span>
              <span className={styles.pickerDescription}>{selectedLength?.wordCount}</span>
              <span className={styles.pickerArrow}>▼</span>
            </div>
          )}
        </Picker>
  ...
  const renderArticleTypeSelector = () => {
    const selectedType = ARTICLE_TYPES.find(type => type.value === config.articleType);
    ...
    return (
      <div className={styles.section}>
        ...
        <Picker
          ...
          onConfirm={(value) => !disabled && setConfig(prev => ({ ...prev, articleType: value[0] as any }))}
        >
          {(_, actions) => (
            <div className={styles.pickerTrigger} onClick={disabled ? undefined : actions.open}>
        ...
        <>
          ...
          <div className={styles.generateSection}>
            <Button
              ...
              disabled={disabled || (!config.useWeakWords && config.words.length === 0)}
              className={styles.generateButton}
            >
              <AiOutlineBook className={styles.buttonIcon} />
              <span>生成文章</span>
              {!config.useWeakWords && (
                <span className={styles.wordCount}>({config.words.length}个单词)</span>
              )}
              {config.useWeakWords && (
