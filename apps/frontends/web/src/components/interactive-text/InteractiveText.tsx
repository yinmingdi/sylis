...
import { GrammarAnalysisModal } from '../grammar-analysis/GrammarAnalysisModal';
...
export const InteractiveText: React.FC<InteractiveTextProps> = ({
  ...
  // 翻译状态管理
  const { translations, loading, translateSentence, getShowTranslation } =
  ...
  const handleGrammarAnalysis = useCallback(
    (paragraphIndex: number, sentenceIndex: number, sentenceText: string) => {
  ...
  const getSentenceState = useCallback(
    (paragraphIndex: number, sentenceIndex: number): ITSentenceState => {
