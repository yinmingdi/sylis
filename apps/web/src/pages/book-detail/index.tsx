...
import { addLearningBook, getBooks } from '../../modules/books/api';
...
// 词书详情卡片
function BookDetailCard({ book }: { book: GetBooksResDto }) {
  return (
...
const BookDetail = () => {
  ...
  const [book, setBook] = useState<GetBooksResDto | null>(null);
  ...
  useEffect(() => {
    getBooks()
      .then((res) => {
        const found = (res.data as GetBooksResDto[]).find((b: GetBooksResDto) => b.id === id);
        setBook(found || null);
      })
