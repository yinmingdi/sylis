...
const pilotScript = resolve(
  repositoryRoot,
  "tools/lexicon/run-protected-pilot.mjs",
);
const preparationScript = resolve(
  repositoryRoot,
  "tools/lexicon/prepare-pilot-manifest.mjs",
);
const cleanCommitModule = resolve(
  repositoryRoot,
  "tools/lexicon/assert-clean-commit.mjs",
);
...
test("revalidates frozen pilot bytes after approval without regenerating", async () => {
  const root = await mkdtemp(join(tmpdir(), "sylis-lexicon-pilot-"));
  const workspace = join(root, "workspace");
  const script = join(workspace, "tools/lexicon/run-protected-pilot.mjs");
  const copiedPreparationScript = join(
    workspace,
    "tools/lexicon/prepare-pilot-manifest.mjs",
  );
  const copiedCleanCommitModule = join(
    workspace,
    "tools/lexicon/assert-clean-commit.mjs",
  );
  ...
  const manifestPath = join(
    workspace,
    ".work/lexicon-pilot-input/source-manifest.json",
  );
  ...
  await writeFile(
    copiedPreparationScript,
    await readFile(preparationScript, "utf8"),
  );
  await writeFile(
    copiedCleanCommitModule,
    await readFile(cleanCommitModule, "utf8"),
  );
...
`,
  ...
  run("git", ["config", "user.name", "Lexicon Pilot Test"], {
    cwd: workspace,
  });
  ...
  await writeFile(
    ...
    "This untracked file is outside protected pilot ownership.\n",
  );
  run(
    process.execPath,
    [
  ...
  const outputRoot = join(workspace, ".work/lexicon-pilot");
  ...
  assert.match(second.stdout, /exact-200 lexicon pilot passed/);
  assert.deepEqual(JSON.parse(await readFile(fakeStatePath, "utf8")), {
    compileCount: 3,
    aiCompileCount: 2,
  });
  ...
  assert.equal(evidence.evidenceVersion, "sylis.lexicon-pilot-evidence/3");
