...
import { AiOutlineSend, AiOutlineBook, AiOutlineEdit } from 'react-icons/ai';

import type { ParseGrammarReqDto, ParseGrammarResDto } from '@/legacy-dto';

import styles from './index.module.less';
import { parseGrammar } from '../../modules/ai/api';

export interface GrammarAnalysisProps {
  text: string;
  autoAnalyze?: boolean; // 是否自动分析
  onAnalysisComplete?: (result: ParseGrammarResDto) => void; // 分析完成回调
  className?: string;
}
...
const GrammarAnalysis: React.FC<GrammarAnalysisProps> = ({
  ...
  const renderGrammarLegend = () => (
    <div className={styles.grammarLegend}>
      ...
      <div className={styles.legendItem}>
        <div
          className={`${styles.legendCircle} ${styles.predicateCircle}`}
        ></div>
        <span>谓语</span>
      </div>
      <div className={styles.legendItem}>
        ...
        <div className={styles.analysisContent}>
          <p className={styles.meaningExplanation}>
            {result.aiExplanation ||
              '这句话的意思是"你在一个陌生的城市里。"这里的"strange"可以理解为陌生或不熟悉的意思。'}
          </p>
        </div>
      </div>
    );
    ...
    const grammarItems = result.grammarAnalysis || [
      {
        component: '主语',
        text: 'You',
        explanation: '是句子的主语,表示说话的对象。',
      },
      {
        component: '谓语',
        text: 'are',
        explanation: '是系动词,连接主语和表语。',
      },
      {
        component: '宾语',
        text: 'in a strange city',
        explanation: '是表语,描述主语所处的位置和状态。',
      },
    ];
  ...
  return (
    <div className={`${styles.grammarAnalysis} ${className || ''}`}>
      {/* 手动分析按钮 */}
      {!autoAnalyze && (
        <div className={styles.manualAnalysis}>
          <Button
            color="primary"
            size="large"
