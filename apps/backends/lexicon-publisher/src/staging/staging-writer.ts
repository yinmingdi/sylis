...
import type { ArtifactCollectionPath } from "@sylis/lexicon-artifact";
...
import type { ArtifactReadResult } from "../artifact/reader";
...
async function copyBatch(client: Client, rows: StagingRow[]): Promise<void> {
  if (rows.length === 0) return;
  try {
    await client.query("BEGIN");
    const stream = client.query(
      copyFrom(
        'COPY "LexiconStagingRecord" ("publishRunId", "collectionPath", "position", "payloadHash", "payload") FROM STDIN WITH (FORMAT text)',
      ),
    );
...
export async function stageArtifact(
  ...
  const existing = await database.lexiconStagingRecord.groupBy({
    by: ["collectionPath"],
    where: { publishRunId },
    _max: { position: true },
    _count: { _all: true },
  ...
  try {
    const result = await streamArtifact(artifactPath, {
      async onEntity(entity) {
        if (entity.position <= (committedPosition.get(entity.path) ?? -1)) {
          return;
        }
