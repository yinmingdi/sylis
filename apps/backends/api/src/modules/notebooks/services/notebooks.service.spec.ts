...
const item = (id: string, position: number) => ({
  ...
  currentRevision: {
    ...
    headwordTarget: { headwordId: `${id}-headword` },
    entryTarget: null,
    senseTarget: null,
    collocationTarget: null,
  },
});
...
describe("NotebooksService", () => {
  it("locks the notebook and rewrites every position during reorder", async () => {
    ...
    const transaction = {
      ...
      collectedLexicalItem: {
        ...
        findUniqueOrThrow: vi.fn(async () => items[2]),
      },
    };
  ...
  it("NOTEBOOK-001-UNIT does not mutate items when the notebook is not owned by the actor", async () => {
    const transaction = {
