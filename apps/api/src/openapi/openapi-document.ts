...
export type ApiAudience = "admin" | "user";
...
export function createOpenApiDocument(app: INestApplication): OpenAPIObject {
  const document = SwaggerModule.createDocument(
    ...
    new DocumentBuilder()
      .setTitle("Sylis API")
      .setDescription("Sylis user and administration HTTP API")
      .setVersion("0.0.1")
      .addCookieAuth(
        "sylis_session",
  ...
  return { ...document, openapi: "3.1.0" } as OpenAPIObject;
}
...
export function audienceDocument(
  source: OpenAPIObject,
  audience: ApiAudience,
): OpenAPIObject {
  const prefix = audience === "admin" ? "/api/admin/v1" : "/api/v1";
  ...
  if (Object.keys(paths).length === 0) {
    throw new Error(
      `OpenAPI document contains no ${audience} paths below ${prefix}`,
    );
  }
  ...
  return {
    ...
    info: {
      ...source.info,
      title: audience === "admin" ? "Sylis Admin API" : "Sylis User API",
    },
    paths,
    components: {
      ...source.components,
