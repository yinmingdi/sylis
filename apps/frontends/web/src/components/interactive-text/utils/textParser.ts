...
export const splitIntoSentences = (text: string): string[] => {
  ...
  const abbreviationPattern =
    /\b(Mr|Mrs|Ms|Dr|Prof|Rev|St|Ave|Rd|Blvd|Inc|Ltd|Corp|Co|etc|vs|e\.g|i\.e|U\.S|U\.K|A\.D|B\.C)\s*\./gi;
  ...
  while ((match = sentenceRegex.exec(processedText)) !== null) {
    const sentence = processedText
      .slice(lastIndex, match.index + match[1].length)
      .trim();
    if (sentence) {
      // 恢复缩写词中的点号
 ...
 * 将单词配置数组转换为映射表
 */
export const buildWordsConfigMap = (
  words: ITWord[] = [],
): Map<string, ITWord> => {
