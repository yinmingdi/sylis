...
`api.railway.internal`. Keep importer autodeploy disabled and its restart policy
set to `Never`.
...
JWT_EXPIRES_IN=30d
AI_API_KEY=<environment-specific provider key>
AI_BASE_URL=<OpenAI-compatible base URL>
AI_MODEL=<enabled model name>
...
2. Create `release/v1.0.0` from `develop` and validate its CI result.
3. Merge `release/v1.0.0` into `main`; CI completion releases production.
...
6. Change the importer to `ECDICT_DRY_RUN=false`, deploy it manually, then verify
   `DictionaryImportRun` completed and sample words are correct.

Do not run `prisma seed` in Railway. API pre-deploy executes only
